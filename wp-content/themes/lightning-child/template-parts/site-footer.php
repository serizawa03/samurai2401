<footer class="<?php lightning_the_class_name( 'site-footer' ); ?>">
	...
	<p>Copyright © Fit Journal All Rights Reserved.</p>
	</div>
</footer> 
...
jQuery(function($) {
    $('#mv').vegas({
        slides: [
            { src: '<?php echo get_stylesheet_directory_uri(); ?>/img/no1.jpg' },
            { src: '<?php echo get_stylesheet_directory_uri(); ?>/img/no3.jpg' },
            { src: '<?php echo get_stylesheet_directory_uri(); ?>/img/no6.jpg' },
            { src: '<?php echo get_stylesheet_directory_uri(); ?>/img/no7.jpg' }
        ],
				overlay: '<?php echo get_stylesheet_directory_uri(); ?>/js/overlays/02.png', //フォルダ『overlays』の中からオーバーレイのパターン画像を選択
